...
def train(args, atkmodel, tgtmodel, clsmodel, tgtoptimizer, clsoptimizer, target_transform, 
    ...
    for batch_idx, (data, target) in pbar:
        ...
        # Calculate loss
        output, clean_feat = clsmodel(data)
        atkoutput, poison_feat = clsmodel(atkdata)
        sd = clsmodel.state_dict()
        sd_keys = clsmodel.state_dict().keys()
        weight_tensor = clsmodel.state_dict()['linear.weight']
...
def get_train_test_loaders(args):
    ...
    finetune_loader = get_dataloader(args, True, args.pretensor_transform, finetune=True)
...
def create_config_parser():
    ...
    parser.add_argument('--lr-atk', type=float, default=0.0005, help='learning rate for attack model')
    ...
    parser.add_argument('--target_label', type=int, default=0) #only in effect if it's all2one
    parser.add_argument('--eps', type=float, default=0.1, help='epsilon for data poisoning')
    ...
    parser.add_argument('--epochs_per_external_eval', type=int, default=10)
    parser.add_argument('--cls_test_epochs', type=int, default=5)
    parser.add_argument('--path', type=str, default='experiments', help='resume from checkpoint')
